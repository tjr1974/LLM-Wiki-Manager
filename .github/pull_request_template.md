...
- [ ] When the PR changes wiki-manager coordination (**`Makefile`** **`wiki-manager-*`** targets, **`scripts/wiki_manager_fork_delta.py`**, **`scripts/wiki_family_snapshot.py`**, **`scripts/fork_delta_report.py`**, **`ai/schema/wiki_manager_registry.v1.json`**, **`schema/wiki-manager.md`**), ran **`make wiki-manager-list`**, **`make wiki-manager-refresh-dry`**, **`make wiki-manager-snapshot`**, or **`python3 scripts/wiki_manager_fork_delta.py report --dry-run`** with **`WIKI_MANAGER_*`** env paths set locally (optional). Touch **`tests/test_wiki_manager_fork_delta.py`**, **`tests/test_wiki_family_snapshot.py`**, **`tests/test_fork_delta_report.py`**, or **`tests/test_make_fork_delta_compare.py`** when behavior of **`COMPARE=`**, **`--compare-root`**, **`base-vs-manager-*`**, or registry resolution changes. Pytest map: **`schema/wiki-manager.md`** heading **`## Regression tests`**.
