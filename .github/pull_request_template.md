...
- [ ] **Stop condition:** local run matches intent — **`make wiki-check`** (Markdown gates), **`make wiki-ci`** (CI parity for wiki leg), or **`make wiki-all`** (tests + CI + quality gate + **`wiki-restore-runtime`** so **`ai/runtime/`** matches **HEAD**).

## Pre-push hygiene

- [ ] No secrets: **`.env`** and local-only paths stay untracked (see **`.gitignore`**, **`SECURITY.md`**, **`.env.example`**).
- [ ] If **`ai/runtime/`** diffs are only refreshed timestamps or unintended gate churn after **`make wiki-ci`** or bare **`pytest`**, restored with **`make wiki-restore-runtime`** (or committed intentionally). A full **`make wiki-all`** ends with **`wiki-restore-runtime`** so **`ai/runtime/`** should match **HEAD** unless another step rewrote those files.
...
- [ ] When the PR changes wiki-manager coordination (**`Makefile`** **`wiki-manager-*`** targets, **`scripts/wiki_manager_fork_delta.py`**, **`scripts/wiki_family_snapshot.py`**, **`scripts/wiki_manager_sync_status.py`**, **`scripts/fork_delta_report.py`**, **`ai/schema/wiki_manager_registry.v1.json`**, **`schema/wiki-manager.md`**, or **`.github/workflows/ci.yml`** post-**`wiki-test`** **`wiki_manager_sync_status.py`** smoke), ran **`make wiki-manager-list`**, **`make wiki-manager-refresh-dry`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-sync-status-json`**, or **`python3 scripts/wiki_manager_fork_delta.py report --dry-run`** with **`WIKI_MANAGER_*`** env paths set locally (optional). Touch **`tests/test_wiki_manager_fork_delta.py`**, **`tests/test_wiki_family_snapshot.py`**, **`tests/test_wiki_manager_sync_status.py`**, **`tests/test_fork_delta_report.py`**, or **`tests/test_make_fork_delta_compare.py`** when behavior of **`COMPARE=`**, **`--compare-root`**, **`--require-base-compare`**, **`base-vs-manager-*`**, or registry resolution changes. Touch **`tests/test_pipeline_step_order.py`** and **`tests/test_karpathy_bridge_docs.py`** when **`ci.yml`** step order or that smoke step changes. Pytest map: **`schema/wiki-manager.md`** heading **`## Regression tests`**.
