...
# wiki-test: never run "make wiki-test -q" (-q is a bogus make goal; inherited MAKEFLAGS breaks nested make in tests, see tests/conftest.py). README Pre-push, Assistant preamble table, schema/wiki-quickstart.md Pytest and CI, schema/karpathy-llm-wiki-bridge.md Pytest leg, proposed/README.md, schema/AGENTS.md githooks bullet and Pytest subprocess hygiene, scripts/githooks/README.md, make help, tests/conftest.py tests/_resolved_python.py tests/test_build_hub_links.py tests/test_githooks_wiring.py tests/test_makeflags_inheritance.py tests/test_pipeline_step_order.py (including test_ci_yml_wiki_manager_sync_smoke_follows_wiki_test_before_wiki_ci) tests/test_make_wiki_all_recipe.py tests/test_karpathy_bridge_docs.py tests/test_wiki_manager_fork_delta.py tests/test_wiki_family_snapshot.py tests/test_wiki_manager_sync_status.py.
...
help:
	...
	@echo '  (wiki-test: no extra make goals after the target—e.g. make wiki-test -q is invalid; nested make smoke tests drop inherited MAKEFLAGS in tests/conftest.py; subprocess cwd fixtures use tests/_resolved_python.py tests/test_build_hub_links.py; README Pre-push; tests/test_githooks_wiring.py tests/test_makeflags_inheritance.py tests/test_pipeline_step_order.py tests/test_make_wiki_all_recipe.py)'
