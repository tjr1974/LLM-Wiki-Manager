...
**LLM Wiki Manager.** This tree doubles as a **coordination checkout** for sibling domain wikis: register absolute paths via environment variables (**`schema/wiki-manager.md`**, **`ai/schema/wiki_manager_registry.v1.json`**, **`.env.example`**). The registry names the Base Model path env via **`compare_root_env`** (default **`WIKI_MANAGER_COMPARE_ROOT`**). Run **`make wiki-manager-refresh-dry`** first for a no-write smoke (**`list`** + **`snapshot`** + dry-run fork-delta steps). Then **`make wiki-manager-list`**, **`make wiki-manager-fork-delta-full`**, or **`make wiki-manager-fork-delta-from-base`** (the last **fails fast** unless that compare-root env points at **LLM Wiki Base Model**, so the diff left side cannot silently default to this checkout). Per-child bundles land under **`ai/runtime/manager/<id>/`** without overwriting the single-child **`make fork-delta-full CHILD='…'`** artifacts. Export the Base Model path (same env name) when measuring **parent → child** tooling drift. **`make wiki-manager-snapshot`** or **`make wiki-manager-snapshot-json`** inventories all four checkouts when env paths are set. **`make wiki-manager-sync-status`** writes **`ai/runtime/manager/sync_status.min.json`** (snapshot plus optional fork-delta counts after **`wiki-manager-report`** / **`wiki-manager-fork-delta-full`**). Root **`family_snapshot_warning_codes`** mirrors **`family_snapshot.warning_codes`** for compare-root structural issues. **GitHub Actions** **`.github/workflows/ci.yml`** runs **`python3 scripts/wiki_manager_sync_status.py --json`** after **`make wiki-test`** as a lightweight script smoke (no **`WIKI_MANAGER_*`** exports on the runner) and asserts that field is present. **`make wiki-manager-base-vs-manager-report`** diffs **Base Model** against **this** tree so you can port shared tooling into **Manager** before refreshing children. Pytest entry points for those flows sit under **`## Regression tests`** in **`schema/wiki-manager.md`**.
...
| Multi-repo fork-delta (registry + env paths) | **`schema/wiki-manager.md`** (**`## Regression tests`** for pytest paths), **`make wiki-manager-list`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-snapshot-json`**, **`make wiki-manager-report`**, **`make wiki-manager-report-from-base`**, **`make wiki-manager-fork-delta-full`**, **`make wiki-manager-fork-delta-from-base`**, **`make wiki-manager-sync-status`**, **`make wiki-manager-sync-status-json`**, **`make wiki-manager-base-vs-manager-report`**, **`make wiki-manager-base-vs-manager-full`**, **`make wiki-manager-refresh-dry`**, **`.env.example`** (**`WIKI_MANAGER_*`**) |
| Pipeline "meta" correctness or drift | **`tests/test_pipeline_step_order.py`**, **`tests/test_make_wiki_all_recipe.py`**, **`tests/test_karpathy_bridge_docs.py`**, **`tests/test_githooks_wiring.py`**, **`tests/conftest.py`**, **`tests/_resolved_python.py`**, **`tests/test_build_hub_links.py`**, **`tests/test_makeflags_inheritance.py`**, **`schema/human-wiki-automation-boundary.md`**, **`.github/ISSUE_TEMPLATE/wiki-toolchain.md`** |
...
When the diff left side should be a sibling **LLM Wiki Base Model** checkout while policy and JSON outputs stay in this tree, use **`scripts/fork_delta_report.py --compare-root`** (see **`schema/wiki-manager.md`**) or export the Base Model path using the env from registry **`compare_root_env`** (default **`WIKI_MANAGER_COMPARE_ROOT`**) and run **`make wiki-manager-fork-delta-full`**.
