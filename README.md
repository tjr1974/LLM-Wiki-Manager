...
**LLM Wiki Manager.** This tree doubles as a **coordination checkout** for sibling domain wikis: register absolute paths via environment variables (**`schema/wiki-manager.md`**, **`ai/schema/wiki_manager_registry.v1.json`**, **`.env.example`**). Run **`make wiki-manager-refresh-dry`** first for a no-write smoke (**`list`** + **`snapshot`** + dry-run fork-delta steps). Then **`make wiki-manager-list`**, **`make wiki-manager-fork-delta-full`**, or **`make wiki-manager-fork-delta-from-base`** (the last **fails fast** unless **`WIKI_MANAGER_COMPARE_ROOT`** points at **LLM Wiki Base Model**, so the diff left side cannot silently default to this checkout). Per-child bundles land under **`ai/runtime/manager/<id>/`** without overwriting the single-child **`make fork-delta-full CHILD='…'`** artifacts. Set **`WIKI_MANAGER_COMPARE_ROOT`** to a local **Base Model** path when measuring **parent → child** tooling drift. **`make wiki-manager-snapshot`** or **`make wiki-manager-snapshot-json`** inventories all four checkouts when env paths are set. **`make wiki-manager-sync-status`** writes **`ai/runtime/manager/sync_status.min.json`** (snapshot plus optional fork-delta counts after **`wiki-manager-report`** / **`wiki-manager-fork-delta-full`**). **GitHub Actions** **`.github/workflows/ci.yml`** runs **`python3 scripts/wiki_manager_sync_status.py --json`** after **`make wiki-test`** as a lightweight script smoke (no **`WIKI_MANAGER_*`** exports on the runner). **`make wiki-manager-base-vs-manager-report`** diffs **Base Model** against **this** tree so you can port shared tooling into **Manager** before refreshing children. Pytest entry points for those flows sit under **`## Regression tests`** in **`schema/wiki-manager.md`**.
...
| Pipeline "meta" correctness or drift | **`tests/test_pipeline_step_order.py`**, **`tests/test_make_wiki_all_recipe.py`**, **`tests/test_karpathy_bridge_docs.py`**, **`tests/test_githooks_wiring.py`**, **`schema/human-wiki-automation-boundary.md`**, **`.github/ISSUE_TEMPLATE/wiki-toolchain.md`** |
