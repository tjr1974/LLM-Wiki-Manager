...
**LLM Wiki Manager.** This tree doubles as a **coordination checkout** for sibling domain wikis: register absolute paths via environment variables (**`schema/wiki-manager.md`**, **`ai/schema/wiki_manager_registry.v1.json`**, **`.env.example`**). Run **`make wiki-manager-refresh-dry`** first for a no-write smoke (**`list`** + **`snapshot`** + dry-run fork-delta steps). Then **`make wiki-manager-list`** or **`make wiki-manager-fork-delta-full`** to produce per-child fork-delta bundles under **`ai/runtime/manager/<id>/`** without overwriting the single-child **`make fork-delta-full CHILD='…'`** artifacts. Set **`WIKI_MANAGER_COMPARE_ROOT`** to a local **LLM Wiki Base Model** path when the diff left side should be that upstream instead of this checkout. **`make wiki-manager-snapshot`** or **`make wiki-manager-snapshot-json`** inventories all four checkouts when env paths are set. **`make wiki-manager-base-vs-manager-report`** diffs **Base Model** against **this** tree so you can port shared tooling into **Manager** before re-running **`make wiki-manager-fork-delta-full`** for domain children. Pytest entry points for those flows sit under **`## Regression tests`** in **`schema/wiki-manager.md`**.
...
| Multi-repo fork-delta (registry + env paths) | **`schema/wiki-manager.md`** (**`## Regression tests`** for pytest paths), **`make wiki-manager-list`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-snapshot-json`**, **`make wiki-manager-report`**, **`make wiki-manager-base-vs-manager-report`**, **`make wiki-manager-fork-delta-full`**, **`make wiki-manager-base-vs-manager-full`**, **`make wiki-manager-refresh-dry`**, **`.env.example`** (**`WIKI_MANAGER_*`**) |
