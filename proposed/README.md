...
**LLM Wiki Manager** sibling checkout coordination (**`make wiki-manager-refresh-dry`**, **`make wiki-manager-fork-delta-from-base`**, **`make wiki-manager-sync-status`**, **`schema/wiki-manager.md`**) is separate from **`proposed/`** staging. Use **`README.md`** (Assistant preamble) and **`wiki/synthesis/llm-wiki-family-repositories.md`** for env paths and playbook order.
