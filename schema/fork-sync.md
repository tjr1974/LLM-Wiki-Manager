...
- **Pytest nested make hygiene**: **`tests/conftest.py`** clears inherited GNU make **`MAKEFLAGS`** so shell-out smoke tests stay reliable. **`tests/test_makeflags_inheritance.py`** documents question-mode failure. Cross-refs live under **`schema/karpathy-llm-wiki-bridge.md`** (**Pytest leg**) and **`schema/wiki-manager.md`** (**Regression tests**).
