...
**LLM Wiki Manager** is the **default canonical checkout for ongoing shared tooling development** in this family. Full rules sit in **`schema/wiki-manager.md`** under **Canonical development hub**. **LLM Wiki Base Model** stays the **neutral sibling** used with **`COMPARE=`** or the manager compare-root env named by registry **`compare_root_env`** (default **`WIKI_MANAGER_COMPARE_ROOT`**) and should receive **cherry-picked** neutral backports from **Manager** on a maintainer-defined cadence. It is **not** an unattended mirror. Domain child repos keep **`wiki/`** narrative local and still follow the **Parent-child alignment charter** below when you treat **Base Model** as the neutral parent of domain content.
...
- **Optional article quality bookkeeping (fork-local)**. Domain children MAY add optional front matter (**`content_maturity`**, **`authoring_priority`**, review dates, short maintainer notes) strictly for human triage, wire a small validator from **`scripts/validate_wiki_front_matter.py`** when keys are present, and add an emitter (for example **`make wiki-authoring-hints`**) that writes machine indexes only under **`ai/artifacts/authoring/`** (full rows plus a capped **top backlog** rollup). Ratings still require manual reading. Keep **`schema/article-quality-tracking.md`**, **`pytest`** for validators and emitter smoke checks, and theme hub prose in **`wiki/themes/`** when maintainers adopt the pattern. Generalize upstream only after the scaffolding is proved topic-neutral end to end.
...
- **Pytest nested make hygiene**: **`tests/conftest.py`** clears inherited GNU make **`MAKEFLAGS`** so shell-out smoke tests stay reliable. **`tests/test_makeflags_inheritance.py`** documents question-mode failure. **`tests/_resolved_python.py`** and **`tests/test_build_hub_links.py`** cover **`subprocess.run`** with **`cwd`** outside the checkout when **`sys.executable`** can be relative (**`schema/AGENTS.md`** **Pytest subprocess hygiene**, plus the note in that section that **`tests/conftest.py`** and **`tests/_resolved_python.py`** module docstrings cross-link **`MAKEFLAGS`** and **`RESOLVED_PYTHON`**). Cross-refs live under **`schema/karpathy-llm-wiki-bridge.md`** (**Pytest leg**) and **`schema/wiki-manager.md`** (**Regression tests**).
...
When using **LLM Wiki Manager** as a dedicated coordination checkout, **`make wiki-manager-list`**, **`make wiki-manager-refresh-dry`** (no artifact writes), **`make wiki-manager-fork-delta-from-base`** or **`make wiki-manager-fork-delta-full`** (see **`schema/wiki-manager.md`**) run the fork-delta machinery for **each** registered child and write per-child bundles under **`ai/runtime/manager/<id>/`** without clobbering the default **`ai/runtime/fork_delta_*.min.json`** paths. **`make wiki-manager-sync-status`** / **`make wiki-manager-sync-status-json`** only roll **`ai/runtime/manager/sync_status.min.json`** plus embedded snapshot data when reports already exist. They do **not** run **`fork_delta_report.py`**. **`scripts/wiki_family_snapshot.py`** emits **`warnings`** and **`warning_codes`** when compare-root equals the manager path or a managed child path. **`scripts/wiki_manager_sync_status.py`** mirrors codes as top-level **`family_snapshot_warning_codes`** (asserted in **`.github/workflows/ci.yml`** after **`make wiki-test`**).
