...
| **Multi-repo coordination (Manager only)** | **`make wiki-manager-*`** and **`scripts/wiki_family_snapshot.py`** print resolved paths and subsystem fork-delta JSON or Markdown under **`ai/runtime/manager/`** per **`schema/wiki-manager.md`**. Maintainers cherry-pick into each checkout by hand. Same authorship boundary as **`make fork-delta`**. |
