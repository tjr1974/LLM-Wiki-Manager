...
**Parent and child checkout hygiene.** **`schema/fork-sync.md`** and **`schema/wiki-manager.md`** document **`make fork-delta`** with optional **`COMPARE=`**, **`make wiki-manager-list`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-snapshot-json`**, **`make wiki-manager-report`**, **`make wiki-manager-report-from-base`**, **`make wiki-manager-base-vs-manager-report`**, **`make wiki-manager-base-vs-manager-full`**, **`make wiki-manager-fork-delta-full`**, **`make wiki-manager-fork-delta-from-base`**, **`make wiki-manager-sync-status`**, **`make wiki-manager-sync-status-json`**, and **`make wiki-manager-refresh-dry`** so gist-aligned ingest and compile habits stay paired with subsystem-level diff review instead of wholesale merges of domain **`wiki/`** trees. **`schema/wiki-manager.md`** lists pytest entry points under **`## Regression tests`**. The **Canonical development hub** section there states that **LLM Wiki Manager** is the default **canonical development checkout** for shared tooling and that **LLM Wiki Base Model** receives **cherry-picked** neutral backports rather than acting as an unattended mirror.

**Rollup JSON warning codes.** **`scripts/wiki_manager_sync_status.py`** exposes **`family_snapshot_warning_codes`** at the rollup root. **`.github/workflows/ci.yml`** asserts that field after **`make wiki-test`**. **`scripts/wiki_family_snapshot.py`** emits **`warnings`** and **`warning_codes`** when compare-root equals this manager checkout or the same directory as a managed child. See **`wiki/synthesis/llm-wiki-family-repositories.md`**.
