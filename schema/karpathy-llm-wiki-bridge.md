...
**Parent and child checkout hygiene.** **`schema/fork-sync.md`** and **`schema/wiki-manager.md`** document **`make fork-delta`** with optional **`COMPARE=`**, **`make wiki-manager-list`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-snapshot-json`**, **`make wiki-manager-report`**, **`make wiki-manager-base-vs-manager-report`**, **`make wiki-manager-base-vs-manager-full`**, **`make wiki-manager-fork-delta-full`**, and **`make wiki-manager-refresh-dry`** so gist-aligned ingest and compile habits stay paired with subsystem-level diff review instead of wholesale merges of domain **`wiki/`** trees. **`schema/wiki-manager.md`** lists pytest entry points under **`## Regression tests`**. The **Canonical development hub** section there states that **LLM Wiki Manager** is the default **canonical development checkout** for shared tooling and that **LLM Wiki Base Model** receives **cherry-picked** neutral backports rather than acting as an unattended mirror.
