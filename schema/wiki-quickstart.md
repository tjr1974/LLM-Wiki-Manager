...
Optional: **`wiki-manager.md`** documents **`make wiki-manager-list`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-snapshot-json`**, **`make wiki-manager-report`**, **`make wiki-manager-report-from-base`**, **`make wiki-manager-base-vs-manager-report`**, **`make wiki-manager-fork-delta-full`**, **`make wiki-manager-fork-delta-from-base`**, **`make wiki-manager-sync-status`**, **`make wiki-manager-sync-status-json`**, **`make wiki-manager-base-vs-manager-full`**, and **`make wiki-manager-refresh-dry`** when one checkout coordinates several domain child wikis against a shared upstream. The same page lists pytest entry points under **`## Regression tests`** and explains **Canonical development hub** governance (**LLM Wiki Manager** as the default **development root** for shared **`scripts/`**, **`tests/`**, CI, **`Makefile`**, and neutral **`schema/`** work). **`.github/workflows/ci.yml`** runs a **`python3 scripts/wiki_manager_sync_status.py --json`** smoke immediately after **`make wiki-test`** so the rollup script stays importable on every push.
