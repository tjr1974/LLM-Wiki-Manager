...
Optional: **`wiki-manager.md`** documents **`make wiki-manager-list`**, **`make wiki-manager-snapshot`**, **`make wiki-manager-snapshot-json`**, **`make wiki-manager-report`**, **`make wiki-manager-report-from-base`**, **`make wiki-manager-base-vs-manager-report`**, **`make wiki-manager-fork-delta-full`**, **`make wiki-manager-fork-delta-from-base`**, **`make wiki-manager-sync-status`**, **`make wiki-manager-sync-status-json`**, **`make wiki-manager-base-vs-manager-full`**, and **`make wiki-manager-refresh-dry`** when one checkout coordinates several domain child wikis against a shared upstream. Registry **`compare_root_env`** names the Base Model path env when it differs from the default **`WIKI_MANAGER_COMPARE_ROOT`**. The same page lists pytest entry points under **`## Regression tests`** and explains **Canonical development hub** governance (**LLM Wiki Manager** as the default **development root** for shared **`scripts/`**, **`tests/`**, CI, **`Makefile`**, and neutral **`schema/`** work). **`.github/workflows/ci.yml`** runs a **`python3 scripts/wiki_manager_sync_status.py --json`** smoke immediately after **`make wiki-test`** so the rollup script stays importable on every push. That smoke asserts the JSON includes **`family_snapshot_warning_codes`** (see **`schema/wiki-manager.md`**).
...
- **`make wiki-ci`**. **`wiki-compile`** then **`validate_templates.py`** and **`validate_frontend_style.py`**, then the same Markdown gate sequence as **`wiki-check`** (**`validate_wiki_front_matter.py`**, **`validate_wiki.py`**, **`validate_sources_category_index.py`**, **`build_claims.py`**, **`build_coverage_matrix.py`**, **`lint_wiki.py`**, **`validate_human_text.py`**), then **`validate_external_links.py --strict`**, **`validate_human_readiness.py`**, and **`validate_ingest_queue_health.py`** (writes **`ai/runtime/ingest_queue_health.min.json`**. By default it disallows **`st=error`** and **`st=queued`** rows in **`ai/runtime/ingest.queue.ndjson`**). **`make wiki-queue-health`** runs only that gate (pass **`--max-queued-rows`** when a backlog is expected). **`VALIDATE_WIKI_ARGS=--strict-citation-meta`** on **`wiki-validate`**, **`wiki-check`**, or **`wiki-ci`** tightens **`validate_wiki`** without editing the **`Makefile`**. This target does **not** call **`check_quality_gate.py`**. **`.github/workflows/ci.yml`** runs **`make wiki-quality-gate`** immediately after **`make wiki-ci`**.
- **`make wiki-test`**. **`pytest -q`** then **`wiki-restore-runtime`** so **`ai/runtime/`** matches **HEAD** after the test leg (same first target **`make wiki-all`** invokes). When **`pytest`** fails, the restore step is skipped so failures stay inspectable. Run **`make wiki-restore-runtime`** afterward if you still want **`ai/runtime/`** reset to **HEAD**.
- **`make wiki-all`**. **`wiki-test`** then **`wiki-ci`** then **`wiki-quality-gate`** then **`wiki-restore-runtime`**. Same three merge stages as **`.github/workflows/ci.yml`**, plus the final restore only on local full parity. **`wiki-test`** already restores **`ai/runtime/`** before **`wiki-ci`**. The trailing **`wiki-restore-runtime`** drops **`wiki-ci`** timestamp churn so **`git status`** stays clean. A failed **`pytest`** stops **`make wiki-all`** before **`wiki-ci`**.

**Pytest and CI.** **`.github/workflows/ci.yml`** installs **`requirements.txt`** before **`make wiki-test`**, so the **`pytest`** on that runner tracks the pin in **`requirements.txt`**. If your shell **`pytest --version`** is older, create **`.venv`**, run **`.venv/bin/pip install -r requirements.txt`**, then **`PATH=".venv/bin:$PATH" make wiki-test`** or **`make wiki-all`**. **README.md** (Pre-push and **Assistant preamble → repo mechanics** table, **`Makefile`** top comments, **`make help`**), **`proposed/README.md`**, **`karpathy-llm-wiki-bridge.md`** (**Pytest leg**), and **`schema/AGENTS.md`** (githooks bullet) state the same rule. Do not append stray **`make`** flags after **`make wiki-test`** (**`make wiki-test -q`** is wrong). Use **`pytest -q`** inside the venv when you want quiet **`pytest`** output. **`tests/conftest.py`** clears inherited **`MAKEFLAGS`** so nested **`make`** subprocess smoke tests stay reliable (**`tests/test_makeflags_inheritance.py`** documents GNU make question mode in isolation). **`tests/test_make_wiki_all_recipe.py`** pins the local **`make wiki-all`** **`wiki-restore-runtime`** tail (Actions keeps three steps only). Tests that spawn Python via **`subprocess.run`** with **`cwd`** outside the checkout should pass **`tests/_resolved_python.py`** **`RESOLVED_PYTHON`** when **`sys.executable`** might be relative (**`schema/AGENTS.md`** **Pytest subprocess hygiene**, **`tests/test_build_hub_links.py`**, and the note in that section that **`tests/conftest.py`** and **`tests/_resolved_python.py`** module docstrings cross-link **`MAKEFLAGS`** and **`RESOLVED_PYTHON`**).
