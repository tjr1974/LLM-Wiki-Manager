...
**LLM Wiki Manager** can register sibling checkouts via **`WIKI_MANAGER_*`** env vars (**`schema/wiki-manager.md`**). Registry **`compare_root_env`** names the Base Model path env when it differs from the default **`WIKI_MANAGER_COMPARE_ROOT`**. Run **`make wiki-manager-refresh-dry`**, **`make wiki-manager-snapshot`**, or **`make wiki-manager-sync-status`** manually when you want path inventory, a **`sync_status.min.json`** rollup, or dry-run fork-delta steps before a push. Those targets do **not** run inside **`pre-push`** by default.
