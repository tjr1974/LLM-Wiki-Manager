...
| **GitHub Actions (default `ci` workflow)** | After **`make wiki-test`**, runs **`python3 scripts/wiki_manager_sync_status.py --json`** pipe smoke (**`.github/workflows/ci.yml`**) and asserts **`family_snapshot_warning_codes`** is present. Full fork-delta bundles stay local (needs **`WIKI_MANAGER_*`** paths). |
...
| 4b | **`make wiki-manager-sync-status`** (after step 4 or **`make wiki-manager-report-from-base`**) writes **`ai/runtime/manager/sync_status.min.json`**: one JSON rollup of Git heads, dirty counts, **`drift_compare_mode`**, per-child **`fork_delta_report`** counts, root **`family_snapshot_warning_codes`**, and nested **`wiki_family_snapshot`** **`warning_codes`** when compare-root equals the manager checkout or the same directory as a managed child. |
